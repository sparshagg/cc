...
def tokenize(code):
    ...
    for mo in re.finditer(tok_regex, code):
        ...
        if kind == 'skip': continue
            ...
            tokens.append(val if val in keywords else 'id')
        elif kind in ['op', 'punc']:
            tokens.append(val)
        elif kind == 'num':
            tokens.append('num')
    tokens.append('$')
...
def parse_ll1(code_string):
    ...
    print("\n" + "="*80)
    print("4. LL(1) PARSING STACK TRACE")
    print("="*80)
...
if __name__ == "__main__":
    ...
    for nt, rules in parse_table.items():
        if rules:
            print(f"Non-Terminal: {nt}")
            for term, prod in rules.items():
                print(f"  On seeing '{term}': use rule [ {nt} -> {' '.join(prod)} ]")
