...
# =================================================================
# 1. THE BUILD SYSTEM & C-COMPILER TRIGGER (WINDOWS COMPATIBLE)
# =================================================================
def build_and_run_c_compiler():
    print("="*65)
    print("🚀 INITIATING MASTER COMPILER PIPELINE (PHASE IV)")
    print("="*65)
    
    print("[*] Step 1: Compiling C Source Code (Bison -> Lex -> GCC)...")
    try:
        # Removed shell=True for Linux/WSL!
        subprocess.run(["bison", "-d", "-y", "par.y"], check=True, stderr=subprocess.DEVNULL)
        # Note: If this fails, change "lex" to "flex" 
        ...
        print("Check your par.y, lex.l, symtab.c, or tac.c files for C syntax errors.")
        sys.exit(1)

    print("[*] Step 2: Running LALR(1), Semantic Analyzer & TAC Generator...")
    
    # Back to standard Linux execution
    ...
    if os.path.exists("quadruples.txt"):
        print("  [+] SUCCESS: Three-Address Code    -> 'quadruples.txt' 🔥")
    if os.path.exists("trace_output.txt"):
        print("  [+] SUCCESS: LALR Internal Trace   -> 'trace_output.txt'")

# =================================================================
# 2. THE LL(1) THEORY ENGINE (GRAMMAR & SCANNER)
# =================================================================
grammar = {
    'PROG': [['SLIST']],
...
def generate_ll1_file(test_code):
    print("[*] Step 3: Generating LL(1) Theory Engine Data...")
    compute_first()
    compute_follow()
    build_table()
...
if __name__ == "__main__":
    ...
    print("="*65)
    print("✨ PIPELINE COMPLETE! All 5 output files are ready.")
    print("="*65)